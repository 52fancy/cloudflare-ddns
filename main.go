...
func main() {
    if len(os.Args) < 5 {
        fmt.Println("Usage: ./ddns CFTOKEN api-token CFZONE_NAME example.com CFRECORD_NAME host.example.com CFRECORD_TYPE A|AAAA|Both")
        os.Exit(1)
    }

    CFTOKEN := os.Args[2]
    CFZONE_NAME := os.Args[4]
    CFRECORD_NAME := os.Args[6]
    CFRECORD_TYPE := os.Args[8]

    // Get Zone ID
    zoneID, err := getZoneID(CFTOKEN, CFZONE_NAME)
    if err != nil {
    ...
    if CFRECORD_TYPE == "A" || CFRECORD_TYPE == "Both" {
        // Get DNS record IP
        dnsRecordIP, err := getDNSRecordIP(CFTOKEN, zoneID, CFRECORD_NAME, "A")
        if err != nil {
            fmt.Println("Error:", err)
            os.Exit(1)
        }
    ...
    if CFRECORD_TYPE == "AAAA" || CFRECORD_TYPE == "Both" {
        // Get DNS record IP
        dnsRecordIP, err := getDNSRecordIP(CFTOKEN, zoneID, CFRECORD_NAME, "AAAA")
        if err != nil {
            fmt.Println("Error:", err)
            os.Exit(1)
        }
    ...
    // Update DNS record
    if ipv4 != "" {
        err = updateDNSRecord(CFTOKEN, zoneID, CFRECORD_NAME, "A", ipv4)
        if err != nil {
            fmt.Println("Error:", err)
            os.Exit(1)
        }
    ...
    if ipv6 != "" {
        err = updateDNSRecord(CFTOKEN, zoneID, CFRECORD_NAME, "AAAA", ipv6)
        if err != nil {
            fmt.Println("Error:", err)
            os.Exit(1)
        }
...
func getZoneID(CFTOKEN, CFZONE_NAME string) (string, error) {
...
func getDNSRecordIP(CFTOKEN, zoneID, CFRECORD_NAME, CFRECORD_TYPE string) (string, error) {
...
func updateDNSRecord(CFTOKEN, zoneID, CFRECORD_NAME, CFRECORD_TYPE, wanIP string) error {
    url := fmt.Sprintf("%s/zones/%s/dns_records?type=%s&name=%s", cloudflareAPIURL, zoneID, CFRECORD_TYPE, CFRECORD_NAME)
    req, err := http.NewRequest("GET", url, nil)
    if err != nil {
        return err
